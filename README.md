...
### 方式一：交给 AI 助手（最简单，推荐给不熟悉命令行的人）

把这个仓库地址发给任意 AI 编程助手，对它说一句话：

```
帮我安装并使用这个项目
https://github.com/SC0511-dev/us-stock-screener
```

支持 Claude Code、Cursor、Codex、Gemini CLI、Windsurf 等——
它们会读到仓库里的 [AGENTS.md](AGENTS.md)，然后：

1. **先问你要分析多大范围的股票**，并说明各档耗时（100 只约 3 分钟 / 520 只约 8 分钟 / 全市场约 50 分钟）
2. 自动装好环境、帮你写好配置
3. 边采集边汇报进度，跑完自动生成可视化报告
4. 之后你可以直接用大白话提需求

装好之后就是纯对话，不用记任何命令：

> 「找出站上 200 日线、相对强弱排前 20% 的股票」
> 「跌下来但基本面还不错的有哪些？」
> 「帮我看看财务最稳健的十只，顺便解释下 F 分是什么」
> 「这批票里谁下周要出财报？」

AI 会把你的话翻译成筛选条件、跑出结果，并解释每个指标的含义与局限。
