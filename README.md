...
- [快速开始](#快速开始)
- [使用方式](#使用方式)
- [为什么不是 A 股系统的翻译版](#为什么不是-a-股系统的翻译版)
- [因子体系](#因子体系)
- [内置策略](#内置策略)
- [数据源](#数据源)
- [项目结构](#项目结构)
...
美股和 A 股的交易制度差异很大，直接套用 A 股因子会得到没有意义的结果。
本项目对每一类因子都做了针对性替换：

| A 股常用 | 美股为何不适用 | 本项目的替代 |
| --- | --- | --- |
| 龙虎榜 | 美股无此披露制度 | **SEC Form 13F 机构持仓** |
| 北向资金 | 陆股通独有 | 机构持股占比与持有家数 |
| 涨跌停板 | 美股无涨跌停 | 跳空缺口 / 波动率 |
| 融资融券余额 | 券商口径不同 | **FINRA 每日做空占比** |
| 逐笔资金流向 | 无免费逐笔数据 | 美元成交额 / OBV / 量价背离 |
| 换手率 | 流通股口径差异大 | 美元成交额（流动性更直接） |
| 回踩年线（250 日） | — | 回踩 50/200 日线（机构参考线） |
| 海龟突破 | 概念泛化、缺乏约束 | **VCP 波动收缩**、**Darvas 箱体** |

并补充了 A 股体系中较少见、但在美股非常核心的因子：
**相对强弱 RS 评级**（对标 IBD）、**52 周高低点位置**、**12-1 动量**、
**自由现金流收益率**、**期权隐含波动率与认沽认购比**、**分析师目标价空间**。
...
项目思路参考了 [myhhub/stock](https://github.com/myhhub/stock)（A 股选股系统 InStock）
的整体架构设计。本项目面向美股重新实现了因子体系与数据层，代码独立编写。
