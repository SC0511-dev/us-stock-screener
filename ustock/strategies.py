...
这里收录的都是**美股市场自身长期使用、且多数有公开研究支撑**的范式，
而不是从 A 股策略直译过来的版本。两个市场的交易制度差异太大：
美股没有涨跌停、可 T+0、允许做空、财报节奏分散、机构持仓每季公开，
因此「涨停板」「龙虎榜跟庄」这类框架在美股要么无从定义，要么含义完全不同。
...
reg(Strategy(
    ...
    desc="达瓦斯箱体法：股价在箱体内震荡后放量突破箱顶。"
         ...
         "替代 A 股语境里常说的「海龟突破」——两者思路相近，"
         "但达瓦斯法更贴合美股无涨跌停、突破后可连续跟进的交易环境。",
...
reg(Strategy(
    ...
    conditions=["above_ma200", "ma_bull_stack", "near_ma50", "rs_strong",
                "liquid_10000000"],
    note="替代 A 股的「回踩年线」。美股以 50/200 日均线为机构参考线，而非 250 日年线。",
))
